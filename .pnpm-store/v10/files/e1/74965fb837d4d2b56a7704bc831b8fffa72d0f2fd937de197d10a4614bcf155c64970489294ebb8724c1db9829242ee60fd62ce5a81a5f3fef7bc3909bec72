...
export const make = /*#__PURE__*/Effect.fnUntraced(function* (options) {
  ...
  const acquireLockConn = sql.onDialectOrElse({
    pg: () => Effect.fnUntraced(function* (scope) {
      const conn = yield* Effect.orDie(sql.reserve).pipe(Scope.provide(scope));
  ...
  const expiresSeconds = sql.literal(Math.ceil(Duration.toSeconds(Duration.fromInputUnsafe(config.shardLockExpiration))).toString());
  ...
  const execWithLockConn = effect => {
    ...
    return lockConn.await.pipe(Effect.flatMap(([conn]) => conn.executeRaw(query, params)), Effect.onError(() => lockConn.rebuildUnsafe()));
  };
  const execWithLockConnUnprepared = effect => {
    if (!lockConn) return effect.values;
    const [query, params] = effect.compile();
    return lockConn.await.pipe(Effect.flatMap(([conn]) => conn.executeUnprepared(query, params, undefined)), Effect.onError(() => lockConn.rebuildUnsafe()));
  };
  const execWithLockConnValues = effect => {
    if (!lockConn) return effect.values;
    const [query, params] = effect.compile();
    return lockConn.await.pipe(Effect.flatMap(([conn]) => conn.executeValues(query, params)), Effect.onError(() => lockConn.rebuildUnsafe()));
  };
  ...
  return RunnerStorage.makeEncoded({
    ...
    release: sql.onDialectOrElse({
      pg: () => {
        ...
        }, Effect.onError(() => lockConn.rebuildUnsafe()), Effect.asVoid, PersistenceError.refail);
      },
      mysql: () => {
        if (disableAdvisoryLocks) {
          return (address, shardId) => sql`DELETE FROM ${locksTableSql} WHERE address = ${address} AND shard_id = ${shardId}`.pipe(PersistenceError.refail);
 ...
 * @category Layers
 */
export const layer = /*#__PURE__*/Layer.effect(RunnerStorage.RunnerStorage)(/*#__PURE__*/make({}));
/**
 * @since 4.0.0
 * @category Layers
 */
export const layerWith = options => Layer.effect(RunnerStorage.RunnerStorage)(make(options));
