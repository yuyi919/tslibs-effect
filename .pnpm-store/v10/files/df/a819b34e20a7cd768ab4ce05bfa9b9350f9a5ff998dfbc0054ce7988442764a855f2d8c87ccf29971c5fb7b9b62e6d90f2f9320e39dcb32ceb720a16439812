...
function handlerToHttpEffect(group, endpoint, context, handler, isRaw) {
  ...
  })).pipe(Effect.withErrorReporting, Effect.catch(error => Effect.orDie(encodeError(error))), Effect.provideContext(context));
}
