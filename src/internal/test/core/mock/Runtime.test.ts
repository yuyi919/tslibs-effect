...
describe("Runtime Polyfills", () => {
	...
	describe("runPromise", () => {
		it("executes effect and returns promise (direct call)", async () => {
			const effect = Effect.map(Effect.context<never>(), (c) => Context.get(c as any, Config).value * 2);
	...
	describe("runSync", () => {
		it("executes synchronous effect (direct call)", () => {
			const effect = Effect.map(Effect.context<never>(), (c) => Context.get(c as any, Config).value);
