...
import { withHelper, buildMemoized } from "../../../core/layer";
	...
	describe("withHelper", () => {
		it.effect("provides layer via provide()", () =>
			Effect.gen(function* () {
				...
				const effect = Effect.map(Effect.context<never>(), (ctx) => Context.get(ctx as any, ServiceA).value);
		...
		it.effect("merges layers via provideMerge()", () =>
			Effect.gen(function* () {
				...
				const layerB = Layer.effect(
					ServiceB,
					Effect.map(Effect.context<never>(), (ctx) => ({ value: Context.get(ctx as any, ServiceA).value * 2 }))
				);
		...
		it.effect("returns empty layer via optional() when failing", () =>
			Effect.gen(function* () {
				...
				const result = yield* Effect.succeed(42).pipe(Effect.provide(optionalLayer));
