...
const flatten = (command: ChildProcess.Command) => {
  ...
  return {
    commands: [
      head,
      ...tail,
    ] as Arr.NonEmptyReadonlyArray<ChildProcess.StandardCommand>,
    opts,
  };
};
...
export const make = Effect.gen(function* () {
  ...
  ): NodeChildProcess.IOType | undefined => (Stream.isStream(x) ? "pipe" : x);
  ...
  const stdin = (
    ...
    const cfg: ChildProcess.StdinConfig = {
      stream: "pipe",
      encoding: "utf-8",
      endOnDone: true,
    };
  ...
  const stdios = (
    ...
    extra: ReadonlyArray<{
      fd: number;
      config: ChildProcess.AdditionalFdConfig;
    }>
  ...
  const setupFds = Effect.fnUntraced(function* (
    ...
    for (const x of extra) {
      ...
      switch (x.config.type) {
        ...
        case "output": {
          let stream: Stream.Stream<Uint8Array, PlatformError.PlatformError> =
            Stream.empty;
  ...
  const setupStdin = (
    ...
    Effect.suspend(() => {
      let sink: Sink.Sink<void, unknown, never, PlatformError.PlatformError> =
        Sink.drain;
      if (Predicate.isNotNull(proc.stdin)) {
        sink = NodeSink.fromWritable({
          evaluate: () => proc.stdin!,
          onError: (err) =>
            toPlatformError("fromWritable(stdin)", toError(err), command),
  ...
  const setupOutput = (
    ...
    let stdout = proc.stdout
      ? NodeStream.fromReadable({
          ...
          onError: (cause) =>
            toPlatformError("fromReadable(stdout)", toError(cause), command),
        })
      : Stream.empty;
    let stderr = proc.stderr
      ? NodeStream.fromReadable({
          ...
          onError: (cause) =>
            toPlatformError("fromReadable(stderr)", toError(cause), command),
        })
      : Stream.empty;
      ...
      return Effect.callback<void, PlatformError.PlatformError>((resume) => {
        NodeChildProcess.exec(
          ...
          (err) => {
            if (err)
              return resume(
                Effect.fail(toPlatformError("kill", toError(err), command))
              );
            resume(Effect.void);
          }
        );
  ...
  const killOne = (
    ...
    Effect.suspend(() => {
      ...
      return Effect.fail(
        toPlatformError(
          "kill",
          new Error("Failed to kill child process"),
          command
        )
      );
    });
  ...
  const source = (
    ...
    switch (opt) {
      ...
      default: {
        ...
        return Predicate.isNotUndefined(fd)
          ? handle.getOutputFd(fd)
          : handle.stdout;
      }
    }
  };
        ...
        const [proc, signal] = yield* Effect.acquireRelease(
          ...
          Effect.fnUntraced(function* ([proc, signal]) {
            ...
            const send = (s: NodeJS.Signals) =>
              Effect.catch(killGroup(command, proc, s), () =>
      ...
      case "PipedCommand": {
        ...
        for (let i = 0; i < tail.length; i++) {
          ...
          const stream = Stream.unwrap(
            Effect.map(handle, (x) => source(x, opts.from))
          );
          ...
          handle = spawnCommand(
            ChildProcess.make(next.command, next.args, {
              ...
              additionalFds: {
                ...
                [ChildProcess.fdName(fd) as `fd${number}`]: {
                  type: "input",
                  stream,
                },
              },
