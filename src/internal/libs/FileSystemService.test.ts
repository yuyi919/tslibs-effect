...
BunTester.layer(live)("AppFileSystem", ({ effect: it }) => {
  describe("isDir", () => {
    it(
      "returns true for directories",
      Effect.gen(function* () {
        const fs = yield* ApplicationFileSystem;
    ...
    it(
      ...
      Effect.gen(function* () {
        const fs = yield* ApplicationFileSystem;
        expect(yield* fs.isDir("/tmp/nonexistent-" + Math.random())).toBe(
          false
        );
      })
    );
  ...
  //     expect(AppFileSystem.mimeType("unknown.qzx")).toBe(
